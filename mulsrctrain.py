...
def get_log(args, model, sample):
    ...
    if torch.cuda.is_available() and torch.cuda.device_count() > 1:
        loss_l2r, nll_loss = nn.parallel.data_parallel(model, sample['net_input'])
        loss_l2r = loss_l2r.sum()
        nll_loss = nll_loss.sum()
    else:
        loss_l2r, nll_loss = model(*sample['net_input'])
    ...
    return loss_l2r, sample_size_l2r, nll_loss, log
