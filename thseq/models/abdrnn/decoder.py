...
class DecoderLayerR2L(nn.Module):
    ...
    def forward(self, inputs, encoder_out, incremental_state):
        ...
        # incremental_state['h'] = x

        return x


class DecoderR2L(nn.Module):
    def __init__(self, args, vocabulary):
        ...
        self.layers.extend([DecoderLayerR2L(args, hidden_size, dropout, args.relu_dropout)
    ...
    def forward(self, inputs, state, is_inference=False):
        ...
        # if is_inference:
        #     x: T x B x H
        # state['h'] = torch.cat([state['h'], x], 0)  # r2l ctx for l2r decoding
        # T x B x H -> B x T x H
...
from thseq.utils.tensor import share_parameters


class Decoder(DecoderBase):
    def __init__(self, args, vocabulary: Vocabulary):
        super().__init__(args, vocabulary)
        self.r2l = DecoderR2L(args, vocabulary)
        self.l2r = DecoderL2R(args, vocabulary)
        self.r2l.embedding = self.l2r.embedding
        self.r2l.logit = self.l2r.logit
        self.r2l.logit.weight = self.r2l.embedding.weight
        if getattr(self.args, 'share_dec', None):
            share_parameters(self.l2r, self.r2l, False)

    def r2l_forward(self, y_r2l, state_r2l):
        logit, state_r2l = self.r2l(y_r2l, state_r2l, False)
        return logit, {
            'ctx': state_r2l['h'],
        }
    ...
    def forward(self, y, state, is_inference=False):
        y_l2r = y
        logit, state = self.l2r(y_l2r, state, is_inference)
        return logit, state
