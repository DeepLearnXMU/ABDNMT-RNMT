...
def smoothed_cross_entropy(logit, target, padding_idx, eps=0.1,reduction='sum'):
    B,T,V=logit.size()
    B_,T_ =target.size()
    assert [B,T]==[B_,T_]
    ...
    mask = target.ne(padding_idx)

    nll_loss = -logp.gather(dim=-1, index=target)[mask]
    smooth_loss = -logp.sum(dim=-1, keepdim=True)[mask]
    if reduction == 'sum':
        nll_loss = nll_loss.sum()
        smooth_loss = smooth_loss.sum()
    eps_i = eps / logp.size(-1)
    loss = (1. - eps) * nll_loss + eps_i * smooth_loss
    return loss, nll_loss


class SmoothedCE_(nn.Module):

    def __init__(self,eps,padding):
        super().__init__()
        self.padding_idx=padding
        self.eps=eps
...
@register_criterion('smoothed-ce')
